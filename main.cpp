...
#if defined(NDEBUG)
# error "Vegtech cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5b656c6ddcbdd879a39cd445b7058496f75a5328db5db270962269cdd3a360dc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vegtech: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vegtech Signed Message:\n";
...
{
    // Vegtech: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Vegtech: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Vegtech: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Vegtech: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vegtech: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vegtech: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9c4859e2cea3e6533ed3ca6ebc20da9d592fd536b4ffe63725508116638110c7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "VegTech was created on the 26th of May 2021";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ee972f9eaccc2a8877417f646bdf97038c612cfaaa463acffbe0c05c04d7d15b60fe924067b402855381d0e5be28217702ea3504baf69ae9ae7ceb12b67bdf3a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622035888;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085974646;

        if (fTestNet)
        {
            block.nTime    = 1622035870;
            block.nNonce   = 386072869;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5965f7ce58508f2377afe74696d99f334932376c260195a559e5622c667c660d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb3, 0xdc }; // Vegtech: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VegtechMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VegtechMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VegtechMiner(CWallet *pwallet)
{
    printf("VegtechMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vegtech-miner");
    ...
    try { loop {
        ...
        printf("Running VegtechMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VegtechMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VegtechMiner, pwallet));
}
